...
function isPlayReviewOtp(phone: string, code: string): boolean {
...
export async function issueOtp(
  ...
  // must not fail issuance. Log failures for visibility/regex tuning.
  try {
    await sendSms(phone, `Your Samity code is ${code}. Valid ${TTL_SEC / 60} minutes.`);
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error(`OTP SMS dispatch issue for ${phone} (code still issued):`, (err as Error).message);
  }
...
export async function consumeOtp(
  ...
  if (!doc) {
    if (isPlayReviewOtp(phone, code)) return undefined;
    throw new ApiError(400, 'VALIDATION_ERROR', 'no active code - request a new one');
  }
